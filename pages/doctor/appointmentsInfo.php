        ...
        <nav class="navbar bg-light navbar-dark text-light ">
            ...
            <div class="btn-group ">
             ...
             <?php
                          ...
                          $date = date('Ymd');
                         $sql2 = mysqli_query($connection, "SELECT * FROM appointment WHERE Date > '$date'");
                         
                         if ($sql2->num_rows > 0) {
                               
                           
                         echo "<table style='background-color : black;'><tr><th>Appointment_ID</th><th>Time</th><th>Date</th><th>DocId</th><th>Cost</th><th>State</th></tr>";
                         // output data of each row
                         while($row = $sql2->fetch_assoc()) {
                            
                         echo "<tr> <td>".$row["AppId"]."</td><td>".$row["Time"]."</td><td>".$row["Date"]."</td><td>".$row["DocId"]."</td><td>".$row["Cost"]."</td><td>".$row["State"]."</td></tr>";
                         }
                         echo "</table>";
                         } else {
